...
class PaymentController extends Controller
{
    public function userDataUpdate($data)
    {
        if($data->status==0)
        {
            ...
            $txt = $data->amount .' USD Deposited Successfully Via '. $data->gateway->name;
            send_email($user->email, $user->name,'Deposit Successfully Completed', $txt);
        }
    ...
    public function depositConfirm()
    {
        ...
        if(is_null($data))
        {
            return redirect()->route('user.deposit')->with('alert', 'Invalid Deposit Request');
        }
        if ($data->status != 0)
        {
            return redirect()->route('user.deposit')->with('alert', 'Invalid Deposit Request');
        ...
        elseif($data->gateway_id ==504)
        {
            ...
            $a = file_get_contents('https://api.coinmarketcap.com/v2/ticker/');
            $doge = json_decode($a, true);
            $btcrate = $doge['data']['74']['quotes']['USD']['price'];
        ...
        elseif($data->gateway_id == 505)
        {
            ...
            if($data->btc_amo==0 ||$data->btc_wallet=="")
            {
                ...
                if ($result['error'] == 'ok') {
                    ...
                    return redirect()->route('user.deposit')->with('alert', 'Failed to Process');
                }
        ...
        elseif($data->gateway_id == 512)
        {
            ...
            {
                return redirect()->route('user.deposit')->with('alert','Unexpected Error! Please Try Again');
                exit();
            }
        ...
        try {
            ...
            try
            {
                ...
                if ($charge['status'] == 'succeeded') {
                    ...
                    //Update User Data
                    $this->userDataUpdate($data);
                    return redirect()->route('user.deposit')->with('success', 'Balance deposited successfully');
                }
    ...
    public function ipnCoinGate()
    {
        ...
        {
            $this->userDataUpdate($data);
            session()->flash('success', 'Balance deposited successfully');
        }
